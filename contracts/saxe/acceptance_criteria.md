...
| `1_discovery/3_brand/tokens.json`             | Visual expectations (colors, spacing — for visual checks only)                  |
...
(list, create, update, delete via standard tRPC endpoints), the generated code
is sufficient and no custom backend testing is needed.
...
### Source Artifacts (same as frontend ACs)
...
| `2_experience/1_journeys/stories.json`         | EARS acceptance criteria, state transitions, guard conditions      |
| `3_blueprint/3_datamodel/postxl-schema.json`  | Model relationships, enum constraints, auth rules                 |
...
2. **Specify the test type.** Use `unit` for service-level logic (tested with
   Jest). Use `e2e` for behavior observable through the UI (tested with Playwright).
