...
def make_bundle(flows_dir: Path, stem: str, content: str) -> Path:
    app_dir = flows_dir / stem
    app_dir.mkdir(parents=True, exist_ok=True)
    p = app_dir / f"{stem}.bundle.yaml"
    p.write_text(content, encoding="utf-8")
    return p


def run_main(flows_dir: Path) -> None:
    compile_main(flows_dir)
...
def test_adds_missing_skill(tmp_path):
    ...
    make_bundle(f, "mvp", "name: mvp\nrequires:\n  - skill:skill-a\n")
    run_main(f)
    content = (f / "mvp" / "mvp.bundle.yaml").read_text()
...
def test_noop_when_bundle_is_already_complete(tmp_path):
    ...
    original = "name: mvp\nrequires:\n  - skill:skill-a\n"
    make_bundle(f, "mvp", original)
    run_main(f)
    assert (f / "mvp" / "mvp.bundle.yaml").read_text() == original
...
def test_skips_skills_covered_by_ancestor(tmp_path):
    ...
    make_bundle(f, "mvp", "name: mvp\nrequires:\n  - skill:skill-a\n")
    make_bundle(f, "simple-app", "name: simple-app\nrequires:\n  - bundle:mvp\n")
    run_main(f)
    content = (f / "simple-app" / "simple-app.bundle.yaml").read_text()
    assert "skill:skill-b" in content
...
def test_exits_1_on_circular_bundle_inheritance(tmp_path):
    ...
    # circular: a → b → a
    make_bundle(f, "a", "name: a\nrequires:\n  - bundle:b\n  - skill:skill-x\n")
    make_bundle(f, "b", "name: b\nrequires:\n  - bundle:a\n  - skill:skill-y\n")
