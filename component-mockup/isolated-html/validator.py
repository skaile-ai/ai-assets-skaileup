#!/usr/bin/env python3
"""Validate produced HTML in `_concept/component-mockup/isolated-html/`.
...
Usage:
  python3 component-mockup/isolated-html/validator.py <out_dir> [--components <dir>]
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        prog="component-mockup-isolated-html-validator",
    ...
    if errors:
        sys.stderr.write(
            f"[component-mockup-isolated-html] FAIL: {len(errors)} violation(s)\n"
        )
        for e in errors:
            sys.stderr.write(f"  - {e}\n")
        return 1
    sys.stdout.write(
        f"[component-mockup-isolated-html] OK: validated {args.out_dir}\n"
    )
    return 0
