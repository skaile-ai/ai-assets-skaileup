...
# ── Helpers ──────────────────────────────────────────────────────────
# Flows and bundles are co-located: flows_dir/<app>/<app>.flow.yaml
...
def write_flow(flows_dir: Path, stem: str, skills: list[str], optional: list[str] | None = None) -> None:
    ...
    (app_dir / f"{stem}.flow.yaml").write_text(yaml.dump(flow), encoding="utf-8")


def write_bundle(flows_dir: Path, stem: str, skills: list[str], parents: list[str] | None = None) -> None:
    requires = [f"bundle:{p}" for p in (parents or [])] + [f"skill:{s}" for s in skills]
    data = {"name": stem, "requires": requires or None}
    app_dir = flows_dir / stem
    app_dir.mkdir(parents=True, exist_ok=True)
    (app_dir / f"{stem}.bundle.yaml").write_text(yaml.dump(data), encoding="utf-8")
...
def test_load_flows_skips_non_skill_type_nodes(tmp_path):
    ...
    (app_dir / "mvp.flow.yaml").write_text(yaml.dump({"id": "mvp", "nodes": nodes, "edges": []}))
    result = load_flows(d)
...
def test_load_flows_deduplicates_duplicate_skill_nodes(tmp_path):
    ...
    (app_dir / "mvp.flow.yaml").write_text(
...
def test_load_bundles_absent_requires_treated_as_empty(tmp_path):
    ...
    app_dir = d / "empty"
    app_dir.mkdir()
    (app_dir / "empty.bundle.yaml").write_text("name: empty\n", encoding="utf-8")
    result = load_bundles(d)
...
def test_load_bundles_preserves_raw_text(tmp_path):
    ...
    app_dir = d / "mvp"
    app_dir.mkdir()
    (app_dir / "mvp.bundle.yaml").write_text(text, encoding="utf-8")
    result = load_bundles(d)
