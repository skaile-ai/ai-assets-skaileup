...
**Source:** [`ops/reverse-engineer/SKILL.md`](https://github.com/skaile-ai/ai-assets-skaileup/blob/main/ops/reverse-engineer/SKILL.md)
:::
...
- `contracts/concept_structure.md` — canonical `_concept/` paths and naming rules
...
- Map framework types → semantic types (see `contracts/semantic_types.md`):
...
| "I'll use the ORM's native types in model.dbml"                                       | Translate to semantic types from `contracts/semantic_types.md`. Stack-specific types belong in stack translations, not the core model.                              |
