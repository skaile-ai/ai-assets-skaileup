...
**Source:** [`impl-architecture/system/SKILL.md`](https://github.com/skaile-ai/ai-assets-skaileup/blob/main/impl-architecture/system/SKILL.md)
:::
...
Show business summary: > "Your app's structure is designed to support [primary business capability]. > [If real-time]: Features like [feature] will show updates instantly using [protocol]. > [If custom modules]: Custom logic handles [business process description]. > [If external integrations]: Your app connects to [service] for [purpose]. > [If no extensions]: The standard [stack] setup covers everything your app needs. > > Technical summary (if interested): > Apps: N, Custom modules: N, Protocols: N, Integrations: N > > Approve, or tell me what to change."
