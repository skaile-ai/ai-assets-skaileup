...
metadata:
  ...
  artifacts:
    ...
    produces:
      - id: goals
        description: 'Success criteria, measurable KPIs, constraints, deadlines, non-goals'
    consumes:
  ...
  prerequisites:
    files:
      - path: '_concept/discovery/brief.md'
        gate: hard
        description: 'Goals are defined against the brief vision and hero flow'
    reads:
      - path: '_concept/discovery/goals.md'
        description: 'Light goals version concept-brief may have written — deepen rather than overwrite'
      - path: '_concept/_grounding/overview/user_input.json'
        description: 'Pre-collected dialog answers (success_criteria field)'
    produces:
      - path: '_concept/discovery/goals.md'
        description: 'Success criteria, KPIs, constraints, deadlines, non-goals'
---
