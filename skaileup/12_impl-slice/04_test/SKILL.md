...
description: "Use when a slice's implement step has finished and you need a per-slice usability gate before proceeding to recap. Runs manual checks + automated tests from plan.md ## Testing strategy, captures user usability observations, emits a Done/NeedsMoreWork/Blocked decision. NOT a project-wide regression suite — for that, use impl-quality/test-{unit,integration,e2e}."
metadata:
  version: "1.0.0"
  tags:
    - impl-slice
