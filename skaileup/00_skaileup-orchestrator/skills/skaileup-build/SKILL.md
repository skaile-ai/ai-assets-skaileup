---
name: skaileup-build
description: 'Use when concept is complete and you want to build the entire application end-to-end. Drives the full pipeline from scaffold through feature implementation to verified, deployable app; journey-first strategy (hero → vital → hygiene). Supports resume from any interrupted phase.'
metadata:
  version: '1.0.0'
  stage: alpha
  tags:
...
`contracts/plans.md`, `contracts/iron_laws.md`, and
`contracts/golden_principles.md` before starting.
...
contracts/golden_principles.md — entity naming and structure rules
...
writes its handoff into `_implementation/slices/<feature_slug>/`:
  ...
  - DO update_progress. The frozen dossier (`index.md` present) marks the slice shipped.
  - EMIT [implement] slice_complete feature=<feature_slug>

  CHECKPOINT slice_<feature_slug>
  > "Feature slice '<feature>' is built, committed, and frozen at
  >  `_implementation/slices/<feature_slug>/`. Approve to start the next feature slice."
