...
description: "Use after techstack.md exists to resolve the abstract stack decision to exactly one concrete scaffold template (template-postxl, template-nextjs-shadcn, template-nextjs-radix, template-nuxt-ui, template-nuxt-primevue, template-nuxt-minimal, template-sveltekit-minimal). Scores each template against the chosen frontend / UI library / backend / database, confirms with the user, and writes the winner back as tech_stack_skill so scaffold and foundation resolve the right TEMPLATE.md."
metadata:
  version: '1.0.0'
  stage: alpha
  tags:
  ...
  artifacts:
    ...
    produces:
      - id: techstack
        description: 'techstack.md with tech_stack_skill resolved to a concrete template-* id'
    consumes:
      - id: scope
        gate: soft
  prerequisites:
    files:
      - path: '_concept/blueprint/techstack.md'
        gate: hard
        description: 'Abstract stack decision must exist before a concrete template can be matched'
    reads:
      - path: '_concept/_meta/scope.yaml'
        description: 'Tier signal — small tiers prefer minimal templates, complex tiers tolerate heavier platforms'
    produces:
      - path: '_concept/blueprint/techstack.md'
        description: 'Same file, tech_stack_skill field updated to the chosen template-* directory id'
---
...
`09_impl-architecture/templates/template-*/`. Downstream skills (`scaffold`,
`foundation`, `design`, the mockup and storybook skills) read
`09_impl-architecture/templates/<tech_stack_skill>/TEMPLATE.md` — so this skill's
...
| Must read        | `09_impl-architecture/templates/template-*/TEMPLATE.md`   | Yes (candidate discovery)|
...
09_impl-architecture/templates/DOMAIN.md — the template cluster and its naming exception

MUST discover candidate templates from 09_impl-architecture/templates/template-\*/TEMPLATE.md at runtime — never hardcode the list
...
- Scan `09_impl-architecture/templates/template-*/TEMPLATE.md`
...
- $ test -d 09_impl-architecture/templates/<id> || echo "ERROR: no such template dir"
...
- [ ] 09_impl-architecture/templates/template-\*/TEMPLATE.md was scanned at runtime
...
- **Feeds into:** `scaffold`, `foundation`, `design`, mockup + storybook skills —
  all resolve `09_impl-architecture/templates/<tech_stack_skill>/TEMPLATE.md`
