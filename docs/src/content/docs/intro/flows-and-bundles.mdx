...
```mermaid
flowchart LR
  subgraph appbuilder-mvp["appbuilder-mvp.flow.yaml"]
    direction LR
...
```mermaid
flowchart LR
  subgraph simple["appbuilder-simple.flow.yaml"]
    direction LR
...
```mermaid
flowchart LR
  subgraph standard["appbuilder-standard.flow.yaml"]
    direction LR
...
```mermaid
flowchart LR
  subgraph complex["appbuilder-complex.flow.yaml"]
    direction LR
...
Larger tiers are supersets of smaller ones by construction (`appbuilder-mvp ⊂ appbuilder-simple ⊂
appbuilder-standard ⊂ appbuilder-complex`), but each flow lists its **own complete** `requires:`
