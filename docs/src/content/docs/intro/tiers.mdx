...
Larger tiers are supersets of smaller ones (`mvp ⊂ simple-app ⊂ standard-app ⊂
complex-app`), but each flow carries its **own complete** `requires:` manifest —
the exact skills its nodes run, no inheritance.

**concept-slice phases by tier:** `standard-app` runs `align → scope-feature → design-feature` (no brainstorm). `complex-app` adds `brainstorm` as the first phase. Both still require `/clear` between phases and read from `_concept/slices/<id>/` dossier files (frozen, kept, on commit).
