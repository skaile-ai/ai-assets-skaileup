...
This page walks the collection end-to-end for a `appbuilder-standard`. (The other tiers
...
flow: appbuilder-standard.flow.yaml
signals:
  features: ~12
  multi_user: true
  persistence: db
...
For `appbuilder-standard` and `appbuilder-complex`, the concept side runs in two passes.
...
appbuilder-complex to catch drift before it compounds.
...
```mermaid
flowchart TD
  scope["scope-project"] --> flow["flows/appbuilder-standard.flow.yaml"]
  ...
  subgraph loop["loop until done"]
    direction TB
    cs["concept-slice<br/>align → scope → design<br/><i>(no brainstorm in appbuilder-standard)</i>"] --> is["impl-slice<br/>brainstorm → align → plan →<br/>implement → test → recap → refactor → commit"]
