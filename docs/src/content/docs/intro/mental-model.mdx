...
flowchart TD
  ...
  scope["<b>skaileup/scope/scope-project</b><br/>2–3 questions: how many features?<br/>single- or multi-user? integrations?<br/>► writes _concept/_meta/scope.yaml {tier, flow}<br/>► picks tier: appbuilder-mvp · simple · standard · complex"]
  scope -->|user can override| flow

  flow["<b>flows/&lt;tier&gt;.flow.yaml</b><br/>graph + requires: manifest<br/>◄ install only what the tier needs"]
  flow --> concept
  flow --> impl
...
¹ concept-slice brainstorm runs only in `appbuilder-complex`; `appbuilder-standard` starts at align.
...
- **`lab/`** — operates on **skills themselves**: `validate`, `judge`, `improve`,
  `learn`, `report`, `compile-validators`, `archive` (collection-agnostic, in
  `ai-assets-skill-development`).
...
flowchart TD
  ...
  catalog["<b>ai-assets-skaileup</b> ← you are here<br/>• SKILL.md / DOMAIN.md / CONTRACT.md<br/>• self-contained flows"]
